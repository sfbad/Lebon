...
app.get('/deposer_annonce/:categorie',(req,res)=>{
  const categorie = req.params.categorie;
    ...
    const subcategoriesForCategory = categoriesWithSubcategories
        .filter(item => item.category === categorie)
        .map(item => item.subcategory);
        console.log(subcategoriesForCategory,categorie)

    res.render(html,{subcategories: subcategoriesForCategory});
}) ;
app.post('/deposer_annonce/:categorie/:subc',(req,res)=>{
  const html =`form`+req.params.categorie ;
  res.redirect('/');
}) ;
